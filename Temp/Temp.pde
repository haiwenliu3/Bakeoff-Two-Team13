...
void draw() {
  ...
  for(int i=0; i<trialCount; i++) {
    ...
    boolean closeZ = abs(t.z - logo_square_width)<inchToPix(.05f); //has to be within +-0.05"
    if (closeDist) {
      stroke(52, 168, 50);
  ...
  rect(screenTransX + width/2, screenTransY + height/2, logo_square_width, logo_square_height);
  
  // draw controls to manipulate the square
  if (!translateMode) {
    // size scaler box
    rect(screenTransX+width/2 + logo_square_width / 2, screenTransY + height/2 + logo_square_height / 2, 20, 20);
  ...
  // resizer
  if (resizingMode) {
    logo_square_width = logo_square_height = constrain(2 * min(mouseX - (screenTransX + width/2), mouseY - (screenTransY + height/2)), 0.01, inchToPix(4f));
    //logo_square_width = mouseX - logo_square_x;
    //logo_square_height = mouseY - logo_square_y;
    PVector mouseV = new PVector(mouseX, mouseY);
    PVector compareV = new PVector(screenTransX + width/2, screenTransY + height/2);
    float angle = PVector.angleBetween(compareV, mouseV);
    System.out.println(angle);
    logo_square_angle = angle - (PI/4);
  }
...
void mouseDragged(MouseEvent evt) {
  if (!translateMode) {
    if (dist(screenTransX + width/2 + logo_square_width / 2, screenTransY + height/2 + logo_square_height / 2, mouseX, mouseY) < 10) {
      resizingMode = true;
    }
  }
}
